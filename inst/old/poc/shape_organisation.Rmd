...
```
```{r}
# colocalisation example: two jobs
yaml_coloc2 <- "
job1:
  variants:
    shape: single_region
...
yaml2 <- yaml::yaml.load(yaml_coloc2)
yaml2
# dataset_coloc <- gwasglue(yaml_coloc)
```
...
```{r}
data_ieu_a_2 <- read.table("data/summary_stats/ieu-a-2_TopHits_sumdata_mod.txt", header=TRUE)
ieu_a_2 <- create_summaryset(data_ieu_a_2, effect_allele_col="effect_allele", other_allele_col="non_effect_allele", pvalue_col="pvalue", position_col="pos")

data_ieu_a_7 <- read.table("inst/tests/ieu-a-7_sumdata_ieu-0-7TopHits.txt", header=TRUE)
ieu_a_7 <- create_summaryset(data_ieu_a_7)

bed_ref <- "data/ld/EUR"

yaml <- job_coloc(traits = c("summaryset:ieu_a_2", "summaryset:ieu_a_7"), region=c("1:400000-4500000","15:400000-4500000"), bfile= bed_ref, plink_bin="plink", build="GRCh37")
cat(yaml)

dataset <- gwasglue_for(yaml)
```
...
yaml <- job_coloc(traits = c("opengwas:ieu-a-2", "opengwas:ieu-a-7"), region=c("1:400000-4500000","15:400000-4500000"), bfile= bed_ref, plink_bin="plink", build="GRCh37")
cat(yaml)
