...
```{r}
ntraits <- getLength(dataset_hmgcr)
ds_hmgcr_marginalised <- lapply(1:ntraits, function(i)
  {
    # Takes in 1 SS
    # Outputs 1 DS (with at least 1 SS)
    ds <- susie_rss(getSummarySet(ds_hmgcr, trait))
}) %>%
...
```{r}
res_simple <- hyprcoloc(ds_hmgcr)
```

Try hyprcoloc with marginalised datasets

```{r}
res_marginalised <- hypercoloc(dds_hmgcr_marginalised)
```
